...
int smsa_vread( SMSA_VIRTUAL_ADDRESS addr, uint32_t len, unsigned char *buf ) {
	...
	if (tptr == NULL) {						// If it isn't
		smsa_operation(get_opcode(0x2, drm, blk), NULL);	// Seek drum
		smsa_operation(get_opcode(0x3, drm, blk), NULL);	// Seek block

		tptr = malloc(256);					// Request Heap memory
		smsa_operation(get_opcode(0x4, drm, blk), tptr); 	// Read into temp -> Will seek to next blk
		smsa_put_cache_line(drm, blk, tptr);			// Cache the data
		indc = 1;						// Update indicator for next step
	} else {
		smsa_operation(get_opcode(0x2, drm, blk), NULL);	// Only Seek for drum [This is handled in a weird way for optimization]
	...
	while (rb < len) {						// Loop until the read bytes are less than the length of buffer
		if (blk == 256) {					// Check if we reach end of drum
			...
			smsa_operation(get_opcode(0x2, drm, blk), NULL);// Seek to the new incremented drum
		}
		...
		if (tptr == NULL) {					// If it isn't
			...
			smsa_operation(get_opcode(0x4, drm, blk), tptr);	// Read into temp
...
int smsa_vwrite( SMSA_VIRTUAL_ADDRESS addr, uint32_t len, unsigned char *buf )  {
	...
	if (tptr == NULL) {					// If it isn't
		...
		smsa_operation(get_opcode(0x3, drm, blk), NULL);	// Seek back to the block
	} else {
		smsa_operation(get_opcode(0x2, drm, blk), NULL);	// Seek to drum
		smsa_operation(get_opcode(0x3, drm, blk), NULL); 	// Seek to block
	}
	...
	smsa_operation(get_opcode(0x5, drm, blk), tptr); 	// Dump data to smsa
	...
	while (wb < len) {
		if (blk == 256) { 				// Check if drum is filled up
			...
			smsa_operation(get_opcode(0x2, drm, blk), NULL);	// Seek to the next drum
			smsa_operation(get_opcode(0x3, drm, blk), NULL);	// Seek to block 0
		}
		...
		if (tptr == NULL) {					// If they aren't, do this
			...
			smsa_operation(get_opcode(0x4, drm, blk), tptr);// backup the data into the ptr
			smsa_put_cache_line(drm, blk, tptr);		// Put data in the drum
			smsa_operation(get_opcode(0x3, drm, blk), NULL);// Seek back only for block
		}
		...
		smsa_operation(get_opcode(0x5, drm, blk), tptr); // Dump temp to smsa
